...
public class Level00: ILevel
{
    public IEnumerator StartLevel()
    {
        ...
        var ShipC0 = Object.Instantiate(ShipByName("TestEnemy"), new Vector2(0f, 2f), Quaternion.Euler(0, 0, 180));
        ...
        ShipC0.SetLocalVelocity(new Vector2(0, 0.5f));

        yield return new WaitUntil(() => ships.TrueForAll(x => x.IsDestroyed));

        while (true)
        {
            var Asteroid = Object.Instantiate(ShipByName("Asteroid"), new Vector3(Random.Range(-6.5f, 6.5f), 5.5f), Quaternion.Euler(0, 0, 180));
